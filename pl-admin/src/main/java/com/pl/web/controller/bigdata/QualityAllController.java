...
//        boolean bb= CSVUtils.genCsv("E:/11.csv", dataList);
		...
		try {
			...
			//文件输出流
             ...
             bw.append("姓名,部门,关键字总数,月度关键字工作时间和,考核得分,月度文件增长量,键盘敲击量总,鼠标敲击量,工作饱和度(加法),工作饱和度(减法),邮箱发送次数,lync通信次数,会议时长,打电话次数,流程次数,平均流程效率,统计时间").append("\r");
             for (int j = 0; j < empQuality.size(); j++) {
            	 String line=empQuality.get(j).getChinaName()+","
            			 +empQuality.get(j).getDepartment() +","
	...
	@RequestMapping("fileQaulity3.do")
