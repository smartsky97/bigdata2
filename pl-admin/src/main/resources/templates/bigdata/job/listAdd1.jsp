...
<html lang="zh-cn">
<head>
<title>员工饱和度加法(8:30-17:30)</title>
...
function detail(mailname,date){
	window.location = "${pageContext.request.contextPath}/lookAddDetails.do?mailname="+mailname+"&date="+date;
}
</script>
</head>
...
<div style="padding:0 20px 10px;">
	<div class="alert">
		<div class="green">
			<h1>工作饱和度列表(加法_8:30-17:30)</h1>
			<form method="post" action="jobStaAddGetDetailCtrl.do">
				<div class="selectWrap">
					部门：<select class="selectStyle" id="department" name="department">
                           <option value="">请选择部门</option>
                        <c:forEach items="${depts}" var="dept">
                           <option value="${dept.id}" <c:if test="${department == dept.id}">selected</c:if>>${dept.department}</option>
                        </c:forEach>
                        </select> 
					    ...
					    <select class="selectStyle" id="employee" name="mailname">
                         ...
                         <c:forEach items="${nums}" var="num">
                         	<option value="${num.cn_name}" <c:if test="${mailname eq num.cn_name }">selected</c:if> >${num.cn_name}</option>
                         </c:forEach>
                        </select><span style="margin-left:10px;">日期：</span>
                        <input id="startime" name="startime" value="${startime}" formate="yyyyMMdd" style="width:190px" placeholder="开始日期" readonly> - 
					<input id="endtime" name="endtime" value="${endtime}" formate="yyyyMMdd" style="width:190px" placeholder="结束日期" readonly>
					<button id="saveButton" type="submit" value="查询">查询</button>
					<button type="submit" formaction="${pageContext.request.contextPath}/JobAddForCsv.do">导出</button>
				</div>
			</form>
		</div>
	</div>
	<div class="content">
	   ...
	   <ul class="pager">
	    <li><a href="jobStaAddGetDetailCtrl.do?pageNum=0&startime=${startime}&endtime=${endtime}&mailname=${mailname}&department=${department}" >首页</a>  </li>
	   	<li><a href="jobStaAddGetDetailCtrl.do?pageNum=${page.currentPage-1}&startime=${startime}&endtime=${endtime}&mailname=${mailname}&department=${department}" >上一页</a></li>
	   	<li>
	   	<select id="mySelect">
             <c:forEach begin="1" end="${page.totalPage}" var="li">
              <option value="jobStaAddGetDetailCtrl.do?pageNum=${li}&startime=${startime}&endtime=${endtime}&mailname=${mailname}&department=${department}" class="" <c:if test="${pageNum == li}">selected</c:if>>${li}
              </option>
              </c:forEach>
          </select>
	   	</li>
	   	<li><a href="jobStaAddGetDetailCtrl.do?pageNum=${page.currentPage+1}&startime=${startime}&endtime=${endtime}&mailname=${mailname}&department=${department}" >下一页</a></li>
	   	<li><a href="jobStaAddGetDetailCtrl.do?pageNum=${page.totalPage}&startime=${startime}&endtime=${endtime}&mailname=${mailname}&department=${department}" >尾页</a></li>
	   </ul>
	</div>
</div>
