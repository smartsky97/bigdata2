...
<html lang="zh-cn">
<head>
<title>员工饱和度减法(8:30-17:30)</title>
...
<div style="padding:0 20px 10px;">
		...
		<div class="green">
			...
			<form method="post" action="jobStaDownGetDetailCtrl.do">
				<div class="selectWrap">
					...
					<button type="submit" formaction="${pageContext.request.contextPath}/JobDownForCsv.do">导出</button>
				</div>
			</form>
		</div>
	</div>
	<div class="content">
	   ...
	   <ul class="pager">
	    <li><a href="jobStaDownGetDetailCtrl.do?pageNum=0&startime=${startime}&endtime=${endtime}&mailname=${mailname}&department=${department}" >首页</a>  </li>
	   	<li><a href="jobStaDownGetDetailCtrl.do?pageNum=${page.currentPage-1}&startime=${startime}&endtime=${endtime}&mailname=${mailname}&department=${department}" >上一页</a></li>
	   	<li>
	   	<select id="mySelect">
             <c:forEach begin="1" end="${page.totalPage}" var="li">
              <option value="jobStaDownGetDetailCtrl.do?pageNum=${li}&startime=${startime}&endtime=${endtime}&mailname=${mailname}&department=${department}" class="" <c:if test="${pageNum == li}">selected</c:if>>${li}
              </option>
              </c:forEach>
          </select>
	   	</li>
	   	<li><a href="jobStaDownGetDetailCtrl.do?pageNum=${page.currentPage+1}&startime=${startime}&endtime=${endtime}&mailname=${mailname}&department=${department}" >下一页</a></li>
	   	<li><a href="jobStaDownGetDetailCtrl.do?pageNum=${page.totalPage}&startime=${startime}&endtime=${endtime}&mailname=${mailname}&department=${department}" >尾页</a></li>
	   </ul>
	</div>
</div>
